...
urlpatterns +=[
    url(r'^accounts/password/reset/$', auth_views.PasswordResetView, {'template_name': 'update_form.html'}),
    url(r'^accounts/login/$', auth_views.LoginView.as_view, {'template_name': 'login.html'}),
    url(r'^user/password/reset/$',auth_views.PasswordResetView,{'post_reset_redirect' : '/user/password/reset/done/'},name="password_reset"),
    url(r'^user/password/reset/done/$',auth_views.PasswordResetDoneView,name="password_reset_done"),
    url(r'^user/password/reset/(?P<uidb64>[0-9A-Za-z]+)-(?P<token>.+)/$',auth_views.PasswordResetConfirmView,{'post_reset_redirect' : '/user/password/done/'},name="password_reset_confirm"),
    url(r'^user/password/done/$',auth_views.PasswordResetCompleteView,name="password_reset_done"),
]
